...
export default function SettingsPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="mb-6">
        <h1 className="font-display text-2xl font-700 text-[--text-primary]">Settings</h1>
        ...
        <div className="flex-1 card p-6">
          ...
          {active === 'profile' && (
            <motion.div key="profile" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              <div>
                <h2 className="font-display font-600 text-[--text-primary] mb-1">Profile</h2>
                <p className="text-xs text-[--text-muted]">Update your personal information</p>
              </div>

              {/* Avatar */}
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-amber-400/15 flex items-center justify-center text-2xl border border-amber-400/20">
          ...
          {active === 'pomodoro' && (
            <motion.div key="pomodoro" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              <div>
                <h2 className="font-display font-600 text-[--text-primary] mb-1">Focus Timer</h2>
                ...
                {[
                  ...
                  <div key={item.label}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="font-mono text-sm text-amber-400">{item.value} min</span>
                    </div>
                    <input
                      type="range"
                      min={item.min}
          ...
          {active === 'notifications' && (
            <motion.div key="notifications" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              <div>
                <h2 className="font-display font-600 text-[--text-primary] mb-1">Notifications</h2>
                <p className="text-xs text-[--text-muted]">Configure alerts and reminders</p>
              </div>

              <div className="space-y-3">
                {[
                  { label: 'Task due reminders', desc: 'Get notified when tasks are due soon', enabled: notifEnabled, set: setNotifEnabled },
                  { label: 'Daily summary', desc: 'Morning summary of your tasks', enabled: false, set: () => {} },
                  { label: 'Pomodoro alerts', desc: 'Sound when focus session ends', enabled: true, set: () => {} },
                ].map((item) => (
                  <div key={item.label} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/4 border border-white/6">
                    <div>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        item.enabled ? 'bg-amber-400' : 'bg-white/10'
                      )}
                    >
                      <span className={cn(
          ...
          {/* Appearance */}
          {active === 'appearance' && (
            <motion.div key="appearance" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              <div>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        i === 0
                          ...
                          : 'border-white/8 hover:border-white/15 opacity-50'
                      )}
                    >
                      <div className={cn(
                        'w-10 h-7 rounded-lg border',
                        i === 0 ? 'bg-obsidian-950 border-white/10' : i === 1 ? 'bg-obsidian-800 border-white/10' : 'bg-white border-black/10'
                      )} />
                      <span className="text-xs text-[--text-secondary]">{t}</span>
                      {i === 0 && <span className="text-[9px] text-amber-400">Active</span>}
                    </button>
                  ))}
                </div>
                <p className="text-xs text-[--text-muted] mt-2">Light & Dim themes coming soon</p>
              </div>

              <div>
                <label className="block text-xs text-[--text-secondary] mb-2">Accent color</label>
                <div className="flex gap-2">
                  {['#fbbf24', '#4ade80', '#60a5fa', '#f472b6', '#a78bfa'].map((c) => (
                    <button
                      key={c}
                      className={cn(
                        'w-7 h-7 rounded-full border-2 transition-all',
                        c === '#fbbf24' ? 'border-white scale-110' : 'border-transparent opacity-60 hover:opacity-90'
                      )}
                      style={{ background: c }}
                    />
                  ))}
                  ...
                  {['Dashboard', 'Kanban', 'List'].map((v) => (
                    <button
                      ...
                      className={cn(
                        ...
                        v === 'Dashboard' ? 'bg-amber-400/10 text-amber-400 border border-amber-400/20' : 'text-[--text-muted] hover:bg-white/5'
                      )}
                    >
                      {v}
                    </button>
          ...
          {active === 'account' && (
            <motion.div key="account" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              <div>
                <h2 className="font-display font-600 text-[--text-primary] mb-1">Account</h2>
                <p className="text-xs text-[--text-muted]">Manage your account security and data</p>
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/4 border border-white/6">
                  <div>
                    <p className="text-sm text-[--text-primary]">Export data</p>
                    <p className="text-xs text-[--text-muted] mt-0.5">Download all your tasks as JSON</p>
                  </div>
                  <button className="btn-ghost text-xs border border-white/8">
                ...
                <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/4 border border-white/6">
                  <div>
                    ...
                    <p className="text-xs text-[--text-muted] mt-0.5">Update your login password</p>
                  </div>
                  <button
                    onClick={async () => {
                      const supabase = createClient()
                    ...
                    className="btn-ghost text-xs border border-white/8"
                  >
                    Send reset email
                  </button>
                </div>
              ...
              <div className="pt-2 border-t border-white/5 space-y-2">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left text-sm text-[--text-secondary] hover:bg-white/5 hover:text-[--text-primary] transition-all"
                >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left text-sm text-coral-400/70 hover:bg-coral-400/8 hover:text-coral-400 transition-all"
                >
